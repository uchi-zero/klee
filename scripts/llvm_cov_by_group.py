...
# -*- coding: utf-8 -*-
# Group-only KLEE coverage tracker.
# Outputs every coverage kind that llvm-cov exposes in its summary:
#   functions, lines, regions, branches, and (when present) instantiations.
#
# For an instrumented native target (-fprofile-instr-generate -fcoverage-mapping)
# and a KLEE output directory containing test*.ktest files, this script replays tests in
# time order (optionally bucketed by --group-sec), incrementally merges coverage profiles,
# and writes one CSV with all metrics after each group.
#
# Latency + UX improvements:
#   - Per-test timeout with hard-kill fallback
#   - Optional MAKEFLAGS='-n' to avoid executing make recipes during replay
#   - Chunked, multi-threaded llvm-profdata merges
#   - Live single-line progress with percentage + timeout/failure counters
#
...
import argparse, csv, json, os, subprocess, sys, math, re, shutil, signal
...
def list_ktests(outdir: Path, skip_early: bool, ktest_suffix: str):
    ...
    for k in sorted(outdir.glob(pattern)):
        if skip_early:
            # works for plain .ktest names (testXXXXXX.early); if you use extra suffixes, disable this flag.
...
def merge_prof_chunked(llvm_profdata: str, inputs, out: Path, tmpdir: Path, chunk_size: int, threads: int):
    """
    ...
    if len(inputs) <= chunk_size:
        cmd = [llvm_profdata, "merge", "-sparse", f"--num-threads={threads}", *inputs, "-o", str(out)]
    ...
    for i in range(0, len(inputs), chunk_size):
        ...
        cmd = [llvm_profdata, "merge", "-sparse", f"--num-threads={threads}", *chunk, "-o", str(part)]
        run(cmd, check=True, capture=False)
        partials.append(str(part))

    # Merge partials into final
    cmd = [llvm_profdata, "merge", "-sparse", f"--num-threads={threads}", *partials, "-o", str(out)]
    run(cmd, check=True, capture=False)
...
def export_summary_all(llvm_cov: str, target: Path, profdata: Path):
    ...
    p = run([llvm_cov, "export", "--summary-only", f"--instr-profile={profdata}", str(target)], check=False)
    if p.returncode == 0:
    ...
    p = run([llvm_cov, "report", "--summary-only", f"--instr-profile={profdata}", str(target)], check=True)
    rows = [ln.rstrip("\n") for ln in p.stdout.splitlines() if ln.strip()]
    hdr_idx = next((i for i, ln in enumerate(rows) if "Filename" in ln and "Coverage" in ln), None)
    tot_idx = next((i for i, ln in enumerate(rows) if ln.strip().startswith("TOTAL")), None)
...
def find_timeout_bin():
    for name in ("timeout", "/usr/bin/timeout"):
        p = shutil.which(name) if name == "timeout" else (name if os.path.exists(name) else None)
        if p:
            return p
    return None
...
def replay_with_timeout(cmd, env, timeout_bin, timeout_sec, kill_after_sec):
    ...
    """
    if timeout_bin:
        full = [timeout_bin, f"--kill-after={int(kill_after_sec)}s", f"{int(timeout_sec)}s"] + cmd
        p = subprocess.run(full, env=env, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def progress_line(done: int, total: int, current: str, timeouts: int, failures: int):
...
def main():
    ap = argparse.ArgumentParser(description="Group-only KLEE coverage CSV (all llvm-cov summary metrics, low-latency).")
    ap.add_argument("--target", required=True, help="Instrumented native binary (-fprofile-instr-generate -fcoverage-mapping)")
    ap.add_argument("--outdir", required=True, help="KLEE output dir containing test*.ktest")
    ap.add_argument("--group-sec", type=float, default=0.0, help="Group ktests by this bucket size in seconds; 0 = one event per ktest.")
    ...
    ap.add_argument("--skip-early-markers", action="store_true", help="Skip ktests that have a sibling testXXXXXX.early marker.")
    ap.add_argument("--ktest-suffix", default="", help="Additional suffix after .ktest to match (e.g., '.A_data'). Default: ''")
    # Latency knobs
    ap.add_argument("--per-test-timeout-sec", type=float, default=15.0, help="Timeout per klee-replay (seconds).")
    ap.add_argument("--kill-after-sec", type=float, default=2.0, help="SIGKILL after this many seconds if SIGTERM didn’t stop the replay.")
    ap.add_argument("--make-no-exec", action="store_true", help="Set MAKEFLAGS='-n' during replay to avoid executing recipes.")
    ap.add_argument("--merge-chunk-size", type=int, default=5000, help="Max inputs per llvm-profdata merge chunk.")
    ap.add_argument("--num-threads", type=int, default=0, help="Threads for llvm-profdata merges (0 = use CPU count).")
    args = ap.parse_args()
    ...
    tmpdir  = covdir / "tmp"
    covdir.mkdir(exist_ok=True); profraw.mkdir(exist_ok=True); tmpdir.mkdir(exist_ok=True)

    # 1) Discover tests and t0
    tests = list_ktests(outdir, args.skip_early_markers, args.ktest_sufix if hasattr(args, "ktest_sufix") else args.ktest_suffix)
    if not tests:
        print("[group] no ktests found"); return
    ...
    # 3) Incremental merge and CSV (with latency improvements)
    cum_prof = None
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as fcsv, \
         open(timed_out_log, "w", encoding="utf-8") as tlog, \
         open(failed_log,    "w", encoding="utf-8") as flog:
        ...
        for gi, (elapsed, ts) in enumerate(groups, 1):
            ...
            for t in ts:
                ...
                rc, to = replay_with_timeout(cmd, env, timeout_bin, args.per_test_timeout_sec, args.kill_after_sec)
                ...
                # live progress
                progress_line(total_replayed, total_tests, t.name, total_timeouts, total_failed)
            ...
            merge_prof_chunked(args.llvm_profdata, raws_this, bucket_prof, tmpdir / "bucket_parts", args.merge_chunk_size, threads)

            # cumulative merge (small)
            if cum_prof is None:
                cum_prof = tmpdir / f"cum_{int(round(elapsed*1e6))}.profdata"
                merge_prof_chunked(args.llvm_profdata, [bucket_prof], cum_prof, tmpdir / "cum_parts_init", args.merge_chunk_size, threads)
            else:
                new_cum = tmpdir / f"cum_{int(round(elapsed*1e6))}.profdata"
                merge_prof_chunked(args.llvm_profdata, [str(cum_prof), str(bucket_prof)], new_cum, tmpdir / "cum_parts", args.merge_chunk_size, threads)
                cum_prof = new_cum
            ...
            fcsv.flush()
            print(f"[group {gi}/{len(groups)}] tests={len(ts)} replayed_total={total_replayed} timeouts={total_timeouts} failed={total_failed}", flush=True)
